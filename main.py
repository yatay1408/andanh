...
from PyQt5.QtWidgets import QMainWindow, QTableWidgetItem, QApplication, QPushButton, QLineEdit, QVBoxLayout, QHBoxLayout, QWidget, QMenu, QMessageBox, QComboBox
import subprocess
import sys
import requests
...
class ProxyManager(QMainWindow):
    ...
    def route_proxies(self):
        conn = sqlite3.connect('proxies.db')
        c = conn.cursor()
        # Chọn các proxy có giá trị speed và latency hợp lệ
        c.execute("SELECT rowid, ip, port, speed, latency FROM proxies WHERE speed IS NOT NULL AND latency IS NOT NULL")
        active_proxies = c.fetchall()

        # Kiểm tra nếu có proxy nào hợp lệ
        if len(active_proxies) == 0:
            QMessageBox.warning(self, "Lỗi", "Không có proxy nào để sắp xếp!")
            return

        # Sử dụng thuật toán Dijkstra dựa trên tốc độ và latency
        graph = [(rowid, speed, latency) for rowid, ip, port, speed, latency in active_proxies]
        # Sắp xếp dựa trên tổng của (latency + 1/speed)
        sorted_proxies = sorted(graph, key=lambda x: x[2] + (1 / x[1]) if x[1] > 0 else float('inf'))

        # Chuẩn bị danh sách rowid cho câu lệnh SQL
        sorted_rowids = [proxy[0] for proxy in sorted_proxies]
        placeholders = ', '.join('?' * len(sorted_rowids))  # Tạo các dấu ? phù hợp với số lượng rowid

        # Truy vấn danh sách proxy đã được sắp xếp theo rowid
        query = f"SELECT protocol, ip, port, user, pass, speed, location, latency, status FROM proxies WHERE rowid IN ({placeholders})"
        c.execute(query, sorted_rowids)
        sorted_proxy_list = c.fetchall()

        # Cập nhật lại bảng hiển thị với mật khẩu được giữ masked
        self.table.setRowCount(len(sorted_proxy_list))  # Đặt lại số lượng hàng dựa trên số proxy đã sắp xếp
        for row_num, proxy in enumerate(sorted_proxy_list):
            for col_num, data in enumerate(proxy):
                if col_num == 4 and data:  # Mask the password again
                    self.passwords[row_num] = data
                    masked_password = '*' * len(data)
                    self.table.setItem(row_num, col_num, QTableWidgetItem(masked_password))
                else:
                    self.table.setItem(row_num, col_num, QTableWidgetItem(str(data) if data is not None else "None"))
        ...
        QMessageBox.information(self, "Thành công", "Định tuyến đã được sắp xếp tự động và bảng đã được cập nhật!")
